...
export const TreeViewItemVariant = cva(
  ...
  "group relative flex h-[28px] cursor-pointer select-none items-center gap-3 text-foreground-light text-sm transition-colors hover:bg-control aria-expanded:bg-transparent data-[state=open]:bg-transparent", // data-[state=open]:bg-control bg state for context menu open
  {
    variants: {
      isSelected: {
        true: "!bg-selection text-foreground", // bg state for context menu open
        false: "",
      },
      isOpened: {
        true: "bg-control",
      ...
      >
        ...
        {isBranch ? (
          <>
            {isLoading ? (
              <Loader2
                className={cn("animate-spin text-foreground-muted")}
                size={14}
              />
            ) : (
              <ChevronRight
        ...
        <form onSubmit={handleSubmit} className={cn(!isEditing && "hidden")}>
          <Input
            ...
            className="block h-7 w-full px-2 py-1 text-sm"
